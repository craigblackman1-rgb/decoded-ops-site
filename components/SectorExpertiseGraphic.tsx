  ...
  return (
    <div className="w-full">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {sectors.map((s, i) => (
          <div key={i} className="flex flex-col items-center gap-2 p-4 rounded-2xl bg-[#FDFDFF] border border-[#C6C5B9]/40 text-center">
            <div className="text-[#62929E]">{s.icon}</div>
            <div className="text-[10px] font-semibold text-[#393D3F] leading-tight whitespace-pre-line">{s.label}</div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {experienceItems.map((item, i) => (
          <div key={i} className={`p-4 rounded-2xl text-center ${i === 0 ? 'bg-[#62929E]' : i === 2 ? 'bg-[#393D3F]' : 'bg-[#546A7B]'}`}>
