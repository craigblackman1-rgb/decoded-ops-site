...
function Dropdown({ label, items }: { label: string; items: { label: string; href: string }[] }) {
  ...
  return (
    <div className="relative" onMouseEnter={handleEnter} onMouseLeave={handleLeave}>
      <button className="flex items-center gap-1 text-[#023047] hover:text-[#219EBC] transition-colors text-sm font-medium">
    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-[#F8F9FA]/95 backdrop-blur-sm border-b border-[#8ECAE6]/30">
      <nav className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          <Link href="/" className="text-2xl font-semibold text-[#023047]">
        ...
        {/* Mobile menu */}
        {mobileOpen && (
          <div className="md:hidden py-4 border-t border-[#8ECAE6]/30 space-y-1">
            {[
