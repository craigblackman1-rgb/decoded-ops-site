export function AuditWheelGraphic() {
  ...
  const areas = [
    { num: '01', label: 'IT\nInfrastructure',   color: '#219EBC' },
    { num: '02', label: 'Software\n& Systems',   color: '#023047' },
    { num: '03', label: 'eCommerce\n& Digital',  color: '#219EBC' },
    { num: '04', label: 'Processes\n& Ops',       color: '#023047' },
    { num: '05', label: 'Growth &\nOpportunity', color: '#219EBC' },
    { num: '06', label: 'AI\nReadiness',          color: '#023047' },
  ];
    ...
    <div className="w-full flex items-center justify-center">
      <svg viewBox="-40 0 480 400" className="w-full max-w-[420px]" aria-hidden="true">
        ...
        {areas.map((area, i) => {
          ...
          return (
            <g key={`label-${i}`}>
              <line x1={lineStart} y1={lineStartY} x2={lineEnd} y2={lineEndY} stroke="#8ECAE6" strokeWidth="1" />
              {lines.map((line, li) => (
                <text key={li} x={lx} y={ly + li * 13 - (lines.length - 1) * 6} textAnchor={anchor} fontSize="9.5" fontWeight="600" fill="#023047" fontFamily="system-ui, sans-serif">
                  {line}
                </text>
              ))}
            </g>
        ...
        {/* Inner ring */}
        <circle cx={cx} cy={cy} r={innerR} fill="#F8F9FA" stroke="#8ECAE6" strokeWidth="1" />
        <circle cx={cx} cy={cy} r={innerR - 10} fill="#023047" />
        ...
        <text x={cx} y={cy + 4} textAnchor="middle" fontSize="10" fontWeight="700" fill="#219EBC" fontFamily="system-ui, sans-serif" letterSpacing="0.5">DAY</text>
        <text x={cx} y={cy + 17} textAnchor="middle" fontSize="8" fill="#8ECAE6" fontFamily="system-ui, sans-serif">6 areas</text>
      </svg>
    </div>
  );
}
